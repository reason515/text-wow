...
def fix_instruction_elseif7():
    ...
    # 检查每一行，如果是 else if 语句，且下一行是缩进的 {
    i = 0
    while i < len(lines) - 1:
        line = lines[i]
        next_line = lines[i + 1] if i + 1 < len(lines) else b''
        
        # 检查是否是 else if 语句（可能跨多行）
        if b'else if' in line and b'strings.Contains' in line:
            # 检查下一行是否是缩进的 {
