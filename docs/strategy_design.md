...
队伍策略系统支持多角色协同作战，每个角色可以配置独立的策略，同时支持队伍级别的协同策略。

### 角色独立策略

每个角色可以配置独立的策略：

- **技能优先级**: 每个角色有自己的技能优先级列表
- **条件规则**: 每个角色有自己的条件规则
- **目标选择**: 每个角色有自己的目标选择策略
...
type TeamStrategy struct {
    TeamID          int
    Characters      []CharacterStrategy  // 每个角色的策略
    SynergyRules   []SynergyRule         // 队伍协同规则
    TargetAllocation string              // 目标分配策略
}
```
