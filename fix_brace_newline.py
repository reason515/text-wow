...
def fix_brace_newline():
    ...
    # 修复: if/else if 语句后换行，然后下一行是 { 的情况
    # 模式: ...)\n\t\t {\r\r\n 应该是 ...) {\n
    pattern1 = rb'(if\s+strings\.Contains\([^)]+\)[^)]*\))\r?\n\t\t\s*\{\r?\r?\n'
    def replace1(m):
        return m.group(1) + b' {\n'
    ...
    if new_content != content:
        changes += 1
        print(f"  修复了if语句后的换行和大括号")
        content = new_content
    
    # 修复: else if 语句后换行，然后下一行是 { 的情况
    pattern2 = rb'(else\s+if\s+strings\.Contains\([^)]+\)[^)]*\))\r?\n\t\t\s*\{\r?\r?\n'
    def replace2(m):
        return m.group(1) + b' {\n'
