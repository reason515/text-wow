...
for i, line in enumerate(lines):
    ...
    # 检测type定义
    if stripped.startswith('type ') and not in_func:
        if final and final[-1].strip() != '' and not final[-1].strip().startswith('type '):
            final.append('')
        final.append(line)
        prev_was_type = True
        prev_was_func = False
    ...
    if is_empty:
        ...
        if i + 1 < len(lines):
            ...
            if (next_stripped.startswith('func ') or 
                (next_stripped.startswith('type ') and not in_func)):
                if final and final[-1].strip() != '':
                    final.append('')
        # 其他空行都移除
    else:
